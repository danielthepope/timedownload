...
Runs every hour by putting it through cron.
"""
...
errorLogfile = '/home/pi/apps/timedownload/errorlog.txt'
...
def errorLog(message):
    message = 'echo "' + str(datetime.datetime.now())[:19] + " " + str(message).replace('"',"'").append('\n\n') + '" >> ' + errorLogfile
    os.system(message)

def email(subject, content):
    osexec('/home/pi/apps/email.sh "trigger@ifttt.com" "' + subject + '" "' + content.replace('"',"'") + '"', False)

def notify(message):
    if __name__ == '__main__':
        email('#textme', message)
    return
